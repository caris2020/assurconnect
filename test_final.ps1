Write-Host "Test final de l'API des fichiers attachés" -ForegroundColor Green
...
try {
    ...
    if ($cases.Count -gt 0) {
        ...
        try {
            ...
            "Ceci est un fichier de test pour vérifier l'upload" | Out-File -FilePath "test_upload.txt" -Encoding UTF8
            
            $form = @{
                file = Get-Item "test_upload.txt"
                description = "Test upload final"
                category = "test"
            }
