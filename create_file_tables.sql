-- Script de création des tables de fichiers pour l'association aux cartes
-- Ce script crée les tables nécessaires si elles n'existent pas

-- 1. Vérifier et créer la table report_files
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'report_files') THEN
        ...
        RAISE NOTICE 'Table report_files créée avec succès';
    ELSE
        RAISE NOTICE 'Table report_files existe déjà';
    END IF;
END $$;

-- 2. Vérifier et créer la table case_attachments
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'case_attachments') THEN
        ...
        RAISE NOTICE 'Table case_attachments créée avec succès';
    ELSE
        RAISE NOTICE 'Table case_attachments existe déjà';
    END IF;
END $$;
...
-- 4. Créer des contraintes de clés étrangères si les tables parent existent
DO $$
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'reports') THEN
        ...
        ELSE
            RAISE NOTICE 'Contrainte FK report_files -> reports existe déjà';
        END IF;
    ELSE
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'insurance_cases') THEN
        ...
        ELSE
            RAISE NOTICE 'Contrainte FK case_attachments -> insurance_cases existe déjà';
        END IF;
    ELSE
...
SELECT 'Création/Mise à jour terminée avec succès!' as status;
